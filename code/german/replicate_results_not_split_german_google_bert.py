...
print('-- Import google-bert model --')
...
print("loading google-bert")

# Verify model and tokenizer
print(f"Tokenizer: {tokenizer}")
print(f"Model loaded: {model_name_google_bert}")
...
model_dir = '../models/google_bert_checkpoints/random_seed_116'
os.makedirs(model_dir, exist_ok=True)

# Save the original model as epoch 0 (baseline)
baseline_checkpoint_path = os.path.join(model_dir, 'finetuned_google_bert_116_epoch_0.pt')
torch.save({
    'epoch': 0,
...
def fine_tune(model, train_dataloader, epochs, tokenizer, device):
    ...
    for epoch_i in range(0, epochs):
        ...
        epoch_checkpoint_path = os.path.join(model_dir, f'finetuned_google_bert_116_epoch_{epoch_i+1}.pt')
        torch.save({
            'epoch': epoch_i+1,
            'model_state_dict': model.state_dict(),
...
torch.save(model.state_dict(), '../models/finetuned_google_bert_116_final.pt')
...
# Save the results
output_file = "../data/output_csv_files/german/results_DE_gender_neutral_google_bert_116.csv"
