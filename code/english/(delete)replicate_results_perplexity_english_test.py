...
# Save model state after pre-association calculation
#model_copy_state = copy.deepcopy(model.state_dict())
...
# Calculate baseline perplexity (if needed)
...
# # Restore model to pre-perplexity state
...
# Clone the model to a completely separate object for validation/perplexity
if 'val_dataloader' in locals():
    print("\nCalculating baseline perplexity before fine-tuning...")
    perplexity_model = BertForMaskedLM.from_pretrained(model_name_bert, 
                                                      output_attentions=False,
                                                      output_hidden_states=False)
    perplexity_model.load_state_dict(model.state_dict())
    perplexity_model.to(device)
    baseline_loss, baseline_perplexity = compute_perplexity(perplexity_model, val_dataloader, device)
    print(f"Baseline Loss: {baseline_loss:.2f}, Perplexity: {baseline_perplexity:.2f}")
    del perplexity_model
...
torch.save(model.state_dict(), '../models/model_with_validation.pt')
print("Model saved with validation influence")

# Calculate post-fine-tuning perplexity on a new clone
if 'val_dataloader' in locals():
    print("Calculating perplexity after fine-tuning...")
    perplexity_model = BertForMaskedLM.from_pretrained(model_name_bert,
                                                      output_attentions=False,
                                                      output_hidden_states=False)
    perplexity_model.load_state_dict(model.state_dict())
    perplexity_model.to(device)
    eval_losses, perplexities = compute_perplexity(perplexity_model, val_dataloader, device)
    print(f"Perplexity on validation set: {perplexities:.2f}")
    del perplexity_model

# Calculate post-fine-tuning perplexity (if needed)
# if calculate_perplexity:
#     eval_losses, perplexities = compute_perplexity(model, val_dataloader, device)
#     print(f"Perplexity on validation set: {perplexities:.2f}")
    
#     # Create a separate model for post-association calculation
#     post_model = type(model)(model.config)
#     post_model.load_state_dict(model.state_dict())
#     post_model.to(device)
    
#     # Calculate post-associations with clean model
#     post_associations = model_evaluation(data, tokenizer, post_model, device)
# else:
#     # Calculate post-associations directly
#     post_associations = model_evaluation(data, tokenizer, model, device) 
...
# Calculate post-associations on the original model
...
# Save the results
output_file = "../data/output_csv_files/english/results_EN_baseline_perplexity_sample_test_5.csv"
