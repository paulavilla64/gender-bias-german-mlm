"""
Randomness vs Training Effect Analysis for English BERT Gender Bias

This script analyzes the impact of random seed variation on gender bias measurements 
in English BERT models. It compares association scores across multiple random seeds 
(42, 116, 387, 1980) to distinguish between genuine training effects and random 
variation in fine-tuning outcomes.

Features:
- Loads association scores from multiple random seed experiments
- Calculates means and standard deviations across seeds for each gender category
- Compares results against original paper benchmarks (pre/post scores)
- Visualizes individual seed results, averages, and variability measures
- Generates publication-ready plot with error bars and statistical annotations

Output: Comparative visualization showing training consistency across random seeds
Usage: Run to assess whether observed bias changes are due to training or randomness
"""
