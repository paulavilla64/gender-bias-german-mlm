"""
English BERT Perplexity Evaluation Across Training Checkpoints

This script evaluates language modeling performance of BERT models at different training 
stages by computing perplexity on AG News validation data. It processes all saved 
checkpoints from fine-tuning experiments to assess model quality progression.

Features:
- Sets up reproducible environment with fixed random seeds
- Loads AG News dataset for perplexity evaluation
- Processes baseline model (epoch 0 or pre-trained) and all training checkpoints
- Computes masked language modeling perplexity for each checkpoint
- Handles both structured checkpoints and direct state dictionaries
- Calculates improvement metrics relative to baseline performance
- Supports final model evaluation if available

Output:
- CSV file with perplexity scores for each checkpoint
- Performance improvement analysis compared to baseline
- Summary statistics across all evaluated models

Usage: Run after fine-tuning to assess language modeling quality at each training stage
"""
...
eval_corpus = pd.read_csv('../../datasets/ag_news_common_crawl/ag_news_common_crawl.tsv', sep='\t') 
eval_data = []
...
if os.path.exists(final_model_path):
    ...
    final_model = BertForMaskedLM.from_pretrained(model_name,
                                        output_attentions=False,
                                        output_hidden_states=False)
...
results_file = f"../../results/association_files/english/results_{name}_{seed}_perplexity.csv"
