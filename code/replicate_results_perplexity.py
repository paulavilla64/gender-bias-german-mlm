...
# data = data.head(200)
# print("Loaded first 200 rows of the dataset:")
#print(data)
...
def compute_baseline_loss(model, val_dataloader, device):
    ...
    for batch in val_dataloader:
        ...
        logits = outputs[0]  # Extract logits (predictions before softmax)
...
def fine_tune(model, train_dataloader, val_dataloader, epochs, tokenizer, device):
    ...
    for epoch_i in range(0, epochs):
        ...
        for batch in val_dataloader:
            print(f"Batch input_ids shape: {batch[0].shape}")  # Should be [50, 128]
            print(f"Batch labels shape: {batch[2].shape}")  # Should be [50, 128]
            ...
            print(f"b_input_ids shape: {b_input_ids.shape}")  # Expected: [50, 128]
            print(f"b_labels shape: {b_labels.shape}")        # Expected: [50, 128]





            # If labels have shape [100, 128], something is wrong before this point. They are 50, 128, so all good until here.

            #print(f"Batch val_labels shape before moving to GPU: {b_labels.shape}") 

            #num_masks_per_batch = (val_tokens_masked == tokenizer.mask_token_id).sum(dim=1)  # Count per batch
            #total_num_masks = num_masks_per_batch.sum().item()  # Total across all batches

            # print(f"Masked tokens per batch: {num_masks_per_batch.tolist()}")
            # print(f"Total masked tokens: {total_num_masks}")

            # if total_num_masks == 0:
            #     print("⚠️ WARNING: No masked tokens found in this batch! Masking logic might be broken.")

            # Count valid labels (non -100 values)
            valid_labels_count = (val_labels != -100).sum().item()
            print(f"\n🔍 Total valid labels (non -100) in batch: {valid_labels_count}")

            if valid_labels_count == 0:
                print("⚠️ WARNING: No valid labels found in validation set! Loss will be NaN.")
            
            # Ensure that padding tokens are ignored in loss computation
            b_labels[b_input_ids == tokenizer.pad_token_id] = -100  # Fix scope to batch
            ...
            # Ensure batch size matches before reshaping
            #correct_labels = val_labels[:logits.shape[0], :logits.shape[1]]  # Ensure same batch & seq len

            # Print before reshaping
            print(f"Before reshaping:")
            print(f"  Logits shape: {logits.shape}")  # Should be [batch_size, seq_len, vocab_size]
            print(f"  b_labels shape: {b_labels.shape}")  # Should be [batch_size, seq_len]

            # Reshape correctly
            reshaped_labels = b_labels.reshape(-1)  # Should be (6400)

            reshaped_logits = logits.view(-1, logits.size(-1)) # should be (6400, 30522)

            # Print after reshaping
            print(f"After reshaping:")
            print(f"  Logits shape: {reshaped_logits.shape}")  # Should be [batch_size * seq_len, vocab_size]
            print(f"  Labels shape: {reshaped_labels.shape}")
...
# make able to handle
# tune_data = tune_data[:200]
# print("Loaded first 200 rows of the finetuning dataset:")
...
# print("Loaded first 200 rows of the validation dataset:")
#print(eval_data)
...
print("val_tokens shape before masking:", val_tokens.shape)
...
print(f"Sample 0 labels shape: {val_data[0][2].shape}")  # Should be [128]
print(f"Sample 1 labels shape: {val_data[1][2].shape}")  # Should be [128]

for i in range(5):  # Check first 5 samples
    print(f"Sample {i} labels shape: {val_data[i][2].shape}")


num_masks = (val_tokens_masked == tokenizer.mask_token_id).sum().item()
if num_masks == 0:
    print("⚠️ WARNING: No masked tokens found! Masking logic might be broken.")


# SequentialSampler: Used for validation to process data in order (no shuffling)
val_sampler = SequentialSampler(val_data)

# Assuming val_data is already created
print("🔎 Checking dataset sample shapes before batching...")

for i in range(5):  # Check first 5 samples
    input_ids, attention_mask, labels = val_data[i]
    print(f"Sample {i} - input_ids shape: {input_ids.shape}")  # Expected: [128]
    print(f"Sample {i} - labels shape: {labels.shape}")  # Expected: [128]
...
# Save the results
output_file = "../data/output_csv_files/english/results_EN_baseline_perplexity_full.csv"
